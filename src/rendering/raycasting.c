...
void	draw_background(t_game *game)  //Changed from draw.c to raycasting.c because it was on loop overwriting the wall's textures
{
	int	x;
	int	y;
	...
	while (++y < SCREEN_HEIGHT)
	{
		while (++x < SCREEN_WIDTH)
		{
			if (y > SCREEN_HEIGHT / 2)
				put_pixil(game, x, y, game->floor_color); //lower half of the screen print floor color
			else
				put_pixil(game, x, y, game->ceiling_color); //upper half of the screen print floor color
		}
		x = -1;
	}
}
...
static void	init_raycast(t_game *game, int x)
{
	...
	game->ray.dir_x = game->player.dir_x + game->player.plane_x //Ray direction in x (left to right based on camera_x value)
		* game->ray.camera_x;
...
int	raycast(t_game *game)
{
	...
	horizontal_pixels = 0;
	handle_keys(game); //Check for movement before loading the window
	draw_background(game); //Draw the ceiling and floor (not the walls)
