...
static void	check_collision(t_game *game, t_2d_grid checker)
{
	if (game->map.grid[checker.y][checker.x] != '0'
		&& !ft_strrchr("NSWE", game->map.grid[checker.y][checker.x])) //Added NSWE because otherwise, it would spawn walls in the players position
	{
...
static void	hit_length(t_game *game)
{
	...
	if (game->ray.side == 1)
		game->ray.perp_wall_dist = game->ray.side_dist_x //Distance traveled along the ray X
...
static void	draw_walls_data(t_game *game)
{
	game->draw.line_height = fabs((double)SCREEN_HEIGHT
			/ game->ray.perp_wall_dist); //The further the wall is, the smaller it will be
...
static void	each_delta_step(t_game *game)
{
	...
	{
		game->ray.side = 1;
		game->ray.side_dist_x += game->ray.delta_dist_x; //current position in X to use "check_collision"
		game->map.checker.x += game->ray.step_x; //current grid map position
	}
	else
	{
		game->ray.side = 3;
