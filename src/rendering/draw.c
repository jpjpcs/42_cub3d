...
int	darken_color(int color, double perp_wall_dist)
{
	...
	perp_wall_dist /= 5; //Make it darker, does not change colors
	if (perp_wall_dist < 1)
		return (color);
	...
	return (r << 16 | g << 8 | b); //Bitwise back to the original place
}
...
void	draw(t_game *game, int x)
{
	...
	while (y < game->draw.end)
	{
		...
		color = get_pixil(&game->img_text[game->ray.reached_wall], //get the address of the pixel
				game->ray.hit, tex_y);
